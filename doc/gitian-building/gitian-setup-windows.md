...
The build script we'll be using is contained in the Ultragate github repository ([contrib/gitian-build.py](https://github.com/ultranatum/ultragate/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Ultragate repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/ultranatum/ultragate/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, ultragate-detached-sigs, and ultragate GitHub repos.
