...
*Setup instructions for a gitian build of Ultragate Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Ultragate
...
to the Ultragate GitHub release page.
...
- [Building Ultragate Core](#building-ultragate-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ultragate-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ultragate-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ultragate-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ultragate-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ultragate-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ultragate-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ultragate-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ultragate.net) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ultragate-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ultragate-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ultragate-osx-*-build.assert
```
