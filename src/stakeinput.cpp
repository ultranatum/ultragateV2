...
bool CUlgStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CUlgStake from zerocoin spend", __func__);
...
bool CUlgStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CUlgStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CUlgStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CUlgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CUlgStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CUlgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CUlgStake::GetUniqueness() const
{
    //The unique identifier for a ULG stake is the outpoint
...
CBlockIndex* CUlgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CUlgStake::ContextCheck(int nHeight, uint32_t nTime)
{
    ...
    if (!consensus.HasStakeMinDepth(nHeight, nTime, nHeightBlockFrom, nTimeBlockFrom)) {
