...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_3_STAKING_SKIP_MN_SYNC,            4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 ULG
...
// Ultragate: on startup load spork values from previous session if they exist in the sporkDB
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        // Ultragate: add to spork database.
