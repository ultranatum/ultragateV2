...
public:
    ...
    bool WriteZULGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZULGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZULGSeed_deprecated(uint256& seed);
    bool EraseZULGSeed();
    bool EraseZULGSeed_deprecated();

    bool WriteZULGCount(const uint32_t& nCount);
    bool ReadZULGCount(uint32_t& nCount);
