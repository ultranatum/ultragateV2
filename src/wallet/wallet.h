...
#include "zulg/zulgmodule.h"
#include "zulg/zulgwallet.h"
#include "zulg/zulgtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_ULTRAGATE = 61000, // inherited version..
...
// Possible states for zULG send
enum ZerocoinSpendStatus {
    ZULG_SPEND_OKAY = 0,                            // No error
    ZULG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZULG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZULG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZULG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZULG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZULG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZULG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZULG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZULG_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZULG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZULG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZULG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZULG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZULG_SPENT_USED_ZULG = 14,                      // Coin has already been spend
    ZULG_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZULG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void dozUlgRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZULG);
    ...
    bool CreateZULGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zULG wallet
    CzULGWallet* zwalletMain{nullptr};
    std::unique_ptr<CzULGTracker> zulgTracker{nullptr};
    void setZWallet(CzULGWallet* zwallet);
    CzULGWallet* getZWallet();
