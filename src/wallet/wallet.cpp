...
/**
 * Fees smaller than this (in uulg) are considered zero fee (for transaction creation)
        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zULG seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZULG = GetBoolArg("-zapwallettxes", false);
    if (fCheckZULG)
        zulgTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zULG upgrade is active.
            dozUlgRescan(pindex, block, setAddedToWallet, consensus, fCheckZULG);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-ultragate-address
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in ULG) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(_("Wallet needed to be rewritten: restart Ultragate Core to complete"));
            return nullptr;
        } else {
    ...
    if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
        if (prev_version <= FEATURE_PRE_ULTRAGATE && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_ULTRAGATE);
        }
    ...
    CzULGWallet* zwalletInstance = new CzULGWallet(walletInstance);
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zULGWallet
        uiInterface.InitMessage(_("Syncing zULG wallet..."));

        //Load zerocoin mint hashes to memory
        walletInstance->zulgTracker->Init();
