...
const char * const ULTRAGATE_CONF_FILENAME = "ultragate.conf";
const char * const ULTRAGATE_PID_FILENAME = "ultragate.pid";
const char * const ULTRAGATE_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
#else
    const char* pszModule = "ultragate";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ultragate2";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ultragateParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ultragateParams
    // Mac: ~/Library/Application Support/ultragateParams
    // Unix: ~/.ultragate-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UltragateParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "UltragateParams";
#else
    // Unix
    return pathRet / ".ultragate-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", ULTRAGATE_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", ULTRAGATE_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ultragate.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ultragate.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", ULTRAGATE_PID_FILENAME));
