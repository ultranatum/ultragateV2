...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ULG);
    ...
    if (settings.contains("nAnonymizeUltragateAmount") || reset)
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ultragate.cpp during shotdown to also remove the window positions
