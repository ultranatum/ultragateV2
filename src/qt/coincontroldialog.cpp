...
void CoinControlDialog::updateLabels()
{
    ...
    int nDisplayUnit = BitcoinUnits::ULG;
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (haveDest) {
                ...
                // if listMode or change => show Ultragate address. In tree mode, address is not shown again for direct wallet address outputs
