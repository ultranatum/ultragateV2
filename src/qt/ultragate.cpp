...
#include "config/ultragate-config.h"
#endif
...
#include "qt/ultragate/splash.h"
#include "qt/ultragate/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ultragate-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ultragate.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ultragate.qrc)
...
/** Class encapsulating Ultragate Core startup and shutdown.
...
/** Main Ultragate application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (ULTRAGATEGUI)
...
{
    window = new ULTRAGATEGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ULTRAGATEGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ULTRAGATEGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ULTRAGATEGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ULTRAGATEGUI::DEFAULT_WALLET);
        ...
        // Ultragate: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ULTRAGATEGUI::handlePaymentRequest);
        connect(window, &ULTRAGATEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Ultragate can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ultragate_locale);
    Q_INIT_RESOURCE(ultragate);
    ...
    /// 6. Determine availability of data directory and parse ultragate.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Ultragate Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Ultragate Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ultragate: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Ultragate Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
