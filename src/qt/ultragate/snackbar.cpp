...
#include "qt/ultragate/forms/ui_snackbar.h"
#include "qt/ultragate/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ULTRAGATEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ULTRAGATEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
