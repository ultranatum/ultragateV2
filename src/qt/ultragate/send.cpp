...
#include "qt/ultragate/forms/ui_send.h"
#include "qt/ultragate/addnewcontactdialog.h"
#include "qt/ultragate/qtutils.h"
#include "qt/ultragate/sendchangeaddressdialog.h"
#include "qt/ultragate/optionbutton.h"
#include "qt/ultragate/sendconfirmdialog.h"
#include "qt/ultragate/myaddressrow.h"
#include "qt/ultragate/guitransactionsutils.h"
...
SendWidget::SendWidget(ULTRAGATEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-ulg");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination ulgAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(ulgAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ulgAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
