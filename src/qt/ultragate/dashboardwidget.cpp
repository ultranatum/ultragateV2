...
#include "qt/ultragate/dashboardwidget.h"
#include "qt/ultragate/forms/ui_dashboardwidget.h"
#include "qt/ultragate/sendconfirmdialog.h"
#include "qt/ultragate/txrow.h"
...
DashboardWidget::DashboardWidget(ULTRAGATEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ULG earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareUlg, "square-chart-ulg");
    setCssProperty(ui->labelUlg, "text-chart-ulg");
    ...
    setCssProperty(ui->labelAmountUlg, "text-stake-ulg-disable");
	ui->labelAmountMNRewards->setText("0 ULG");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ULTRAGATEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &ULTRAGATEGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &ULTRAGATEGUI::openFAQ);
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isUlg = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ULG, zULG
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1 = new QBarSet("mn" + QString(CURRENCY_UNIT.c_str()));
    set0->setColor(QColor(252,194,0));
    set1->setColor(QColor(201,155,0));
    ...
    set0->append(chartData->valuesUlg);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalUlg > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountUlg, "text-stake-ulg");
	...
	forceUpdateStyle({ui->labelAmountUlg, ui->labelAmountMNRewards});
    ui->labelAmountUlg->setText(GUIUtil::formatBalance(chartData->totalUlg, nDisplayUnit));
