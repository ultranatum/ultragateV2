...
#include "qt/ultragate/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/ultragate/forms/ui_topbar.h"
#include "qt/ultragate/loadingdialog.h"
#include "qt/ultragate/lockunlock.h"
#include "qt/ultragate/qtutils.h"
#include "qt/ultragate/receivedialog.h"
...
TopBar::TopBar(ULTRAGATEGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopUlg}, "amount-small-topbar");
    setCssProperty({ui->labelAmountUlg}, "amount-topbar");
    setCssProperty({ui->labelPendingUlg, ui->labelImmatureUlg, ui->labelAvailableUlg,
                       ui->labelLockedUlg, ui->labelMasternodeCount, ui->labelCollateralUlg,
    ...
    ui->pushButtonConf->setButtonText("ultragate.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open ultragate.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Ultragate")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralUlg->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // ULG
    // Top
    ui->labelAmountTopUlg->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    // Expanded
    ui->labelAmountUlg->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableUlg->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingUlg->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureUlg->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedUlg->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
