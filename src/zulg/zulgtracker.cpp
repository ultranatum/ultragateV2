...
#include "zulg/zulgwallet.h"


CzULGTracker::CzULGTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzULGTracker::~CzULGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzULGTracker::Init()
{
...
bool CzULGTracker::Archive(CMintMeta& meta)
{
...
bool CzULGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzULGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzULGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzULGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzULGTracker::GetSerialHashes()
{
...
CAmount CzULGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzULGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzULGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzULGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzULGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzULGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzULGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzULGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzULGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzULGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzULGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzULGWallet* zULGWallet)
{
    bool iszULGWalletInitialized = (nullptr != zULGWallet);
    CMintMeta meta;
    ...
    if (!iszULGWalletInitialized)
        zULGWallet = new CzULGWallet(wallet);
    meta.isSeedCorrect = zULGWallet->CheckSeed(dMint);
    if (!iszULGWalletInitialized)
        delete zULGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzULGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzULGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzULGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzULGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzULGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzULGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzulg from DB\n", __func__, listDeterministicDB.size());
    }
