...
bool CLegacyzUlgStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyzUlgStake from non zc-spend", __func__);
    ...
    *this = CLegacyzUlgStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zulg stake origin", __func__);

    // All good
    return true;
}

CLegacyzUlgStake::CLegacyzUlgStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyzUlgStake::GetIndexFrom()
{
...
CAmount CLegacyzUlgStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyzUlgStake::GetUniqueness() const
{
...
bool CLegacyzUlgStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zULG stake block: height %d outside range", __func__, nHeight);
