...
CzULGWallet::CzULGWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zulg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZULGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZULGSeed_deprecated()) {
                    LogPrintf("%s: Updated zULG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zulg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zulg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzULGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZULGCount(nCountLastUsed);
    else if (!walletdb.ReadZULGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzULGWallet::Lock()
{
    seedMaster.SetNull();
}

void CzULGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzULGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZULG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzULGWallet::LoadMintPoolFromDB()
{
...
void CzULGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzULGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzULGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zulgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzULGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZULG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zulgTracker which also adds to database
    wallet->zulgTracker->Add(dMint, true);
...
void CzULGWallet::SeedToZULG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzULGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzULGWallet::GenerateDeterministicZULG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzULGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzULGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzULGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
