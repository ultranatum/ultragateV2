...
{
    CDataStream serializedCoinSpend = ZULGModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZULGSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zULG supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zULG supply
    ...
    // A one-time event where only the zULG supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZULGSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zULG supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zULG supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
