...
{
    // double check that there are no double spent zULG spends in this block or tx
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zULG spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zULG check to not include duplicated serials in the same block.
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("UltragateMiner : generated block is stale");
    }
...
void CheckForCoins(CWallet* pwallet, const int minutes, std::vector<COutput>* availableCoins)
{
    ...
    if ((nTimeNow - nMintableLastCheck > minutes * 60)) {
        ...
        fMasternodeSync = sporkManager.IsSporkActive(SPORK_3_STAKING_SKIP_MN_SYNC) || !masternodeSync.NotCompleted();
    }
}

void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    LogPrintf("UltragateMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ultragate-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running UltragateMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("UltragateMiner exception");
    } catch (...) {
        LogPrintf("UltragateMiner exception");
    }

    LogPrintf("UltragateMiner exiting\n");
}
