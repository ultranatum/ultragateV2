...
from test_framework.test_framework import UltragateTestFramework
...
class ReorgStakeTest(UltragateTestFramework):
    ...
    def check_money_supply(self, expected_ulg, expected_zulg):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected ULG and zULG supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_ulg))
            for denom in node['zULGsupply']:
                assert_equal(node['zULGsupply'][denom], DecimalAmt(expected_zulg[denom]))
    ...
    def run_test(self):
        ...
        # Check ULG and zULG supply at the beginning
        # ------------------------------------------
        # zULG supply: 2 coins for each denomination
        expected_zulg_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # ULG supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zulg_supply)
        ...
        # Verify that ULG and zULG supplies were properly updated after the spends and reorgs
        self.log.info("Check ULG and zULG supply...")
        ...
        expected_zulg_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zulg_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zulg_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zulg_supply)
