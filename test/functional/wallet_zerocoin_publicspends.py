...
class ZerocoinSpendTest(UltragateTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zulg_bal, ulg_bal):
            zulg_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zulg_bal)
            ulg_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], ulg_bal)
            return zulg_bal, ulg_bal
        ...
        zulg_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zulg_balance, 6666)
        ...
        zulg_balance, balance = check_balances(denom_2, zulg_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zulg_balance, balance = check_balances(denom_3, zulg_balance, balance)
        ...
        _, _ = check_balances(denom_1, zulg_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
